...
type AddStockItemModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onItemAdded: (newItem: StockItemFormValues) => void;
};

export function AddStockItemModal({ isOpen, onClose, onItemAdded }: AddStockItemModalProps) {
  ...
  const onSubmit = (values: StockItemFormValues) => {
    startTransition(async () => {
      try {
        const result = await addStockItem(values);
        if (result.success) {
          toast({
        ...
        toast({
          variant: 'destructive',
          title: 'Error',
          description: 'Failed to add the stock item. Please try again.',
        });
      }
    });
  };
